    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                // 3. Call API
                const response = yield client.generateCode(description, language);
                if (response.success && response.data.generated_code) {
                    ...
                    logger_1.Logger.log(`Generated code for: ${description}`);
                }
                else {
                    const errorMsg = response.error || 'Unknown error occurred';
                    logger_1.Logger.error(`Generation failed: ${errorMsg}`);
                    vscode.window.showErrorMessage(`Synthex Error: ${errorMsg}`);
                }
            }
            catch (error) {
                logger_1.Logger.error('Generation command failed', error);
